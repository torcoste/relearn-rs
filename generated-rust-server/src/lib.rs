...
pub enum ListQuestionsResponse {
    ...
    ListOfAllTheUsers(Vec<models::Question>),
}
...
pub trait ApiNoContext<C: Send + Sync> {
    ...
    /// Create a new answer
    async fn create_answer(&self) -> Result<CreateAnswerResponse, ApiError>;

    /// Health check
    async fn health(&self) -> Result<HealthResponse, ApiError>;

    /// List all questions
    async fn list_questions(&self, limit: Option<i64>) -> Result<ListQuestionsResponse, ApiError>;
}
