        ...
        C: Has<XSpanIdString>  + Send + Sync + 'static
    {
        ...
        match &method {
            ...
            &hyper::Method::POST if path.matched(paths::ID_ANSWERS) => {
                                let result = api_impl.create_answer(
                                        &context
                                    ).await;
                                ...
                                response.headers_mut().insert(
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                CreateAnswerResponse::CreatedAnswer
                                                    (body)
                                                => {
            ...
            &hyper::Method::GET if path.matched(paths::ID_QUESTIONS) => {
                ...
                let param_limit = match param_limit {
                    ...
                    None => None,
                                ...
                                let result = api_impl.list_questions(
                                            param_limit,
                                        &context
                                    ).await;
                                let mut response = Response::new(Body::empty());
                                response.headers_mut().insert(
                                        ...
                                        match result {
                                            Ok(rsp) => match rsp {
                                                ListQuestionsResponse::ListOfAllTheUsers
                                                    (body)
                                                => {
                                                    *response.status_mut() = StatusCode::from_u16(200).expect("Unable to turn 200 into a StatusCode");
                                                    response.headers_mut().insert(
